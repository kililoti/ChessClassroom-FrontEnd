...
export default function ParticularesPage() {
  ...
  // Efecto para verificar la sesión y cargar las clases grupales al cargar la página
  useEffect(() => {
    ...
    fetch(endpoint)
      ...
      .then((data: Clase[]) => {
        const soloGrupales = data.filter(c => c.tipo === 'grupal');
        setClases(soloGrupales);
        setLoading(false);
      })
      .catch((err) => {
  ...
  // Función para crear una nueva clase grupal (solo para profesores)
        ...
        <div className="p-6 bg-slate-50/50 min-h-[300px] flex flex-col justify-between">
          {clases.length === 0 ? (
            <div className="text-center mt-12">
              <p className="text-slate-500 text-lg">Aún no hay clases grupales registradas.</p>
            </div>
          ) : (
            <>
              ...
              <ul className="space-y-4">
                {clasesPaginadas.map((clase) => (
                  <li key={clase.id} className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-200 transition-all duration-300 gap-4">
                    <div>
                      <span className="font-bold text-lg text-slate-900 block">{clase.nombre}</span>
                      ...
                      <div className="text-sm text-slate-500 mt-2 flex items-center gap-2 flex-wrap">
                        ...
                        {rolUsuario === 'profesor' && (
                          <button
                            onClick={() => handleCopiarEnlace(clase.codigo_invitacion, clase.id)}
                    ...
                    <button 
                      onClick={() => handleToggleEstado(clase.id, clase.activo)}
