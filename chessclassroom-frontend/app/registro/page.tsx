...
function RegisterForm() {
  ...
  return (
    <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
      ...
      {status.error && (
        <div className="mb-4 p-3 text-sm text-red-700 bg-red-100 rounded-lg">{status.error}</div>
      )}
      {status.success && (
        <div className="mb-4 p-3 text-sm text-green-700 bg-green-100 rounded-lg">{status.success}</div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="text" name="apellidos" required value={formData.apellidos} onChange={handleChange}
              className="w-full px-4 py-2 text-black border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
